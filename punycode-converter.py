...
def generate_punycode_variants(letter):
    ...
    if not glyphs:
        console.print(f"❌ No homoglyphs found for '{letter}'.", style="red")
        return

    table = Table(title=f"🔎 Punycode Variants for '{letter}'")
...
def main():
    console.print("=== 🎯 Full Homoglyph Punycode Viewer 🎯 ===", style="bold green")
    console.print("Type 'exit' to quit.\n", style="yellow")

    while True:
        user_input = console.input("[bold blue]Enter a letter, number, or symbol: [/bold blue]").strip()
        if user_input.lower() == "exit":
            console.print("👋 Exiting... Bye!", style="bold red")
            break
        elif len(user_input) == 1:
            generate_punycode_variants(user_input)
        else:
            console.print("❗ Please enter a single character.", style="red")
